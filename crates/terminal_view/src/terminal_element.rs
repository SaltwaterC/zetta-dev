...
impl CursorLayout {
    ...
    fn bounds(&self, origin: GpuiPoint<Pixels>) -> Bounds<Pixels> {
        match self.shape {
            ViewCursorShape::Bar => Bounds::new(self.origin + origin, size(px(2.), self.height)),
            ViewCursorShape::Underline => Bounds::new(
                self.origin + origin + point(Pixels::ZERO, self.height - px(2.)),
                size(self.width, px(2.)),
            ),
            ViewCursorShape::Block | ViewCursorShape::Hollow => {
                Bounds::new(self.origin + origin, size(self.width, self.height))
            }
        }
    }
    ...
    fn paint(&mut self, origin: GpuiPoint<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        window.paint_quad(if matches!(self.shape, ViewCursorShape::Hollow) {
                ...
                let cursor = if let CursorShape::Hidden = cursor.shape {
                    ...
                    ime_cursor_bounds.map(move |bounds| {
                        let (shape, text) = match cursor.shape {
                            ...
                            CursorShape::Bar => (ViewCursorShape::Bar, None),
                            CursorShape::HollowBlock => (ViewCursorShape::Hollow, None),
            ...
            self.interactivity.paint(
                ...
                |_, window, cx| {
                    ...
                    for (relative_highlighted_range, color) in &layout.relative_highlighted_ranges {
                        ...
                        {
                            for (index, line) in highlighted_range_lines.iter().enumerate() {
