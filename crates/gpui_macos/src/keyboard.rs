...
use crate::{
    ...
    kTISPropertyLocalizedName, kTISPropertyUnicodeKeyLayoutData,
};
...
impl MacKeyboardLayout {
    pub(crate) fn new() -> Self {
        // macOS can restart its input-source agent while this notification is in flight.
        // During that window the current source or either property may be unavailable.
        Self::try_new().unwrap_or_else(Self::empty)
    }
    ...
    pub(crate) fn try_new() -> Option<Self> {
        unsafe {
            ...
            // A stale TIS object can keep its identifier and display name even
            // after its keyboard-layout bundle was removed. GPUI cannot
            // translate such a source either: `events::chars_for_modified_key`
            // requires this same `uchr` data. Treat it as unavailable rather
            // than allowing AppKit to retain a context for the invalid source.
            let layout_data = TISGetInputSourceProperty(
                current_keyboard,
                kTISPropertyUnicodeKeyLayoutData as *const c_void,
            );
            let has_layout_data =
                !layout_data.is_null() && !CFDataGetBytePtr(layout_data as CFDataRef).is_null();
            let id =
                input_source_property(current_keyboard, kTISPropertyInputSourceID as *const c_void);
            let name =
                input_source_property(current_keyboard, kTISPropertyLocalizedName as *const c_void);
            let _: () = msg_send![current_keyboard, release];

            Self::from_input_source_properties(has_layout_data, id, name)
        }
    }

    fn from_input_source_properties(
        has_layout_data: bool,
        id: Option<String>,
        name: Option<String>,
    ) -> Option<Self> {
        if !has_layout_data {
...
unsafe fn input_source_property(source: *mut Object, property: *const c_void) -> Option<String> {
    let value: *mut Object = unsafe { TISGetInputSourceProperty(source, property) };
    if value.is_null() {
        return None;
    }
    let value: *const std::os::raw::c_char = msg_send![value, UTF8String];
    if value.is_null() {
        return None;
    }
