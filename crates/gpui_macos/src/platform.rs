...
// `kTISNotify*` notifications are delivered through the distributed center.
    ...
    unsafe {
        APP_DELEGATE_CLASS = {
            ...
            decl.add_method(
                ...
                application_will_resign_active as extern "C" fn(&mut Object, Sel, id),
            );
            decl.add_method(
...
pub(crate) struct MacPlatformState {
    ...
    keyboard_layout_change_pending: bool,
    ...
    /// Prevents AppKit from activating text input while its shared input-source agent restarts.
...
impl MacPlatform {
    pub fn new(headless: bool) -> Self {
        ...
        let keyboard_layout = MacKeyboardLayout::try_new();
        let input_context_gate = Arc::new(InputContextGate::new(keyboard_layout.is_some()));
        let keyboard_layout = keyboard_layout.unwrap_or_else(MacKeyboardLayout::empty);
        ...
        Self(Mutex::new(MacPlatformState {
            ...
            keyboard_layout_change_pending: false,
            ...
            for menu_config in menus {
                ...
                for (item_index, item_config) in menu_config.items.iter().enumerate() {
                        ...
                        && matches!(item_config, MenuItem::Action { .. })
                    {
                        ...
                        let _: () = msg_send![
                            item,
                            setKeyEquivalent: ns_string(&key_equivalent)
                        ];
                        let _: () = msg_send![
                            item,
                            setKeyEquivalentModifierMask: modifier_mask
                        ];
                    }
                    menu.addItem_(item);
                }
    ...
    fn open_window(
        ...
        let (
            ...
            (
                ...
                guard.input_context_gate.clone(),
...
extern "C" fn did_finish_launching(this: &mut Object, _: Sel, _: id) {
    unsafe {
        ...
        // AppKit's keyboard-selection notification is emitted after the text
        // system has started reacting to a change. Observe the authoritative
        // TextInputSources notifications as well, so GPUI stops handing events
        // to an input context as soon as the selected source or enabled-source
        // set changes.
        let distributed_notification_center: id =
            msg_send![class!(NSDistributedNotificationCenter), defaultCenter];
        for name in [
            kTISNotifySelectedKeyboardInputSourceChanged as id,
            kTISNotifyEnabledKeyboardInputSourcesChanged as id,
        ] {
            let _: () = msg_send![distributed_notification_center, addObserver: this as id
                selector: sel!(onKeyboardLayoutChange:)
                name: name
                object: nil
                suspensionBehavior: NS_NOTIFICATION_SUSPENSION_BEHAVIOR_DELIVER_IMMEDIATELY
            ];
        }
...
extern "C" fn application_will_resign_active(this: &mut Object, _: Sel, _: id) {
    let platform = unsafe { get_mac_platform(this) };
    platform.0.lock().input_context_gate.suspend();
}
...
extern "C" fn resume_input_contexts(context: *mut c_void) {
...
extern "C" fn on_keyboard_layout_change(this: &mut Object, _: Sel, _: id) {
    let platform = unsafe { get_mac_platform(this) };
    let mut lock = platform.0.lock();
    lock.input_context_gate.suspend();
    if lock.keyboard_layout_change_pending {
        return;
    }
    lock.keyboard_layout_change_pending = true;
    drop(lock);
    let platform_ptr = platform as *const MacPlatform as *mut c_void;
    unsafe {
        DispatchQueue::main().exec_async_f(platform_ptr, on_keyboard_layout_change_deferred);
    }
}

extern "C" fn on_keyboard_layout_change_deferred(context: *mut c_void) {
    let platform = unsafe { &*(context as *const MacPlatform) };
    let Some(keyboard_layout) = MacKeyboardLayout::try_new() else {
        // Keep the last valid mapper until macOS exposes the replacement source.
        platform.0.lock().keyboard_layout_change_pending = false;
        return;
    };
    let mut lock = platform.0.lock();
    ...
    let app: id = unsafe { msg_send![APP_CLASS, sharedApplication] };
    let is_active: BOOL = unsafe { msg_send![app, isActive] };
    lock.input_context_gate
        .resume_if_active(is_active == YES, || true);
    if let Some(mut callback) = lock.on_keyboard_layout_change.take() {
        drop(lock);
        callback();
        let mut lock = platform.0.lock();
        lock.on_keyboard_layout_change.get_or_insert(callback);
        lock.keyboard_layout_change_pending = false;
    } else {
        lock.keyboard_layout_change_pending = false;
    }
}
...
unsafe extern "C" {
    ...
    pub(super) static kTISNotifySelectedKeyboardInputSourceChanged: CFStringRef;
    pub(super) static kTISNotifyEnabledKeyboardInputSourcesChanged: CFStringRef;
}
