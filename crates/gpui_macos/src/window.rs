use crate::{
    BoolExt, InputContextGate, MacDisplay, NSRange, NSStringExt, TISCopyCurrentKeyboardInputSource,
...
const INPUT_CONTEXT_GATE_IVAR: &str = "inputContextGate";
const TEXT_INPUT_CONTEXT_IVAR: &str = "textInputContext";
const TEXT_INPUT_CONTEXT_CREATING_IVAR: &str = "textInputContextCreating";

static mut WINDOW_CLASS: *const Class = ptr::null();
static mut PANEL_CLASS: *const Class = ptr::null();
static mut VIEW_CLASS: *const Class = ptr::null();
static mut BLURRED_VIEW_CLASS: *const Class = ptr::null();
static mut TEXT_INPUT_CONTEXT_CLASS: *const Class = ptr::null();
...
unsafe fn build_classes() {
    unsafe {
        ...
        TEXT_INPUT_CONTEXT_CLASS = {
            let mut decl =
                ClassDecl::new("GPUITextInputContext", class!(NSTextInputContext)).unwrap();
            decl.add_ivar::<*mut c_void>(INPUT_CONTEXT_GATE_IVAR);
            decl.add_method(
                sel!(dealloc),
                dealloc_text_input_context as extern "C" fn(&Object, Sel),
            );
            decl.add_method(
                sel!(activate),
                activate_text_input_context as extern "C" fn(&Object, Sel),
            );
            decl.add_method(
                sel!(deactivate),
                deactivate_text_input_context as extern "C" fn(&Object, Sel),
            );
            decl.add_method(
                sel!(handleEvent:),
                handle_text_input_context_event as extern "C" fn(&Object, Sel, id) -> BOOL,
            );
            decl.add_method(
                sel!(invalidateCharacterCoordinates),
                invalidate_text_input_context_character_coordinates as extern "C" fn(&Object, Sel),
            );
            decl.register()
        };
        VIEW_CLASS = {
            ...
            decl.add_ivar::<*mut c_void>(INPUT_CONTEXT_GATE_IVAR);
            decl.add_ivar::<id>(TEXT_INPUT_CONTEXT_IVAR);
            decl.add_ivar::<BOOL>(TEXT_INPUT_CONTEXT_CREATING_IVAR);
...
struct MacWindowState {
    ...
    input_context_gate: Arc<InputContextGate>,
...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            let input_context_gate = window.0.lock().input_context_gate.clone();
            (*native_view).set_ivar(
                INPUT_CONTEXT_GATE_IVAR,
                Arc::into_raw(input_context_gate) as *const c_void,
            );
...
impl PlatformWindow for MacWindow {
    ...
    fn update_ime_position(&self, _bounds: Bounds<Pixels>) {
        let (executor, input_context_gate) = {
            let state = self.0.lock();
            (
                state.foreground_executor.clone(),
                state.input_context_gate.clone(),
            )
        };
        if !input_context_gate.is_enabled() {
            return;
        }
        executor
            .spawn(async move {
...
extern "C" fn input_context(this: &Object, _: Sel) -> id {
    let raw: *mut c_void = unsafe { *this.get_ivar(INPUT_CONTEXT_GATE_IVAR) };
    if raw.is_null() {
        // Do not let NSView lazily create its unmanaged context while the
        // view is still being initialized. The GPUI-owned context below is
        // the only one we can safely suspend when the source disappears.
        return nil;
    }

    let input_context_gate = unsafe { &*(raw as *const InputContextGate) };
    if !input_context_gate.is_enabled() {
        // AppKit asks the first responder for this while deactivating input methods.
        // A missing source must not hand AppKit a stale context during that transition.
        return nil;
    }

    let input_context: id = unsafe { *this.get_ivar(TEXT_INPUT_CONTEXT_IVAR) };
    if !input_context.is_null() {
        return input_context;
    }

    let is_creating: BOOL = unsafe { *this.get_ivar(TEXT_INPUT_CONTEXT_CREATING_IVAR) };
    if is_creating == YES {
        // NSTextInputContext initialization can synchronously ask its client
        // for the current context. Returning nil breaks that re-entrant cycle.
        return nil;
    }

    unsafe {
        let this = this as *const Object as *mut Object;
        (*this).set_ivar(TEXT_INPUT_CONTEXT_CREATING_IVAR, YES);
        let input_context: id = msg_send![TEXT_INPUT_CONTEXT_CLASS, alloc];
        let input_context: id = msg_send![input_context, initWithClient: this];
        (*this).set_ivar(TEXT_INPUT_CONTEXT_CREATING_IVAR, NO);
        if input_context.is_null() {
            return nil;
        }
        // The view owns one raw Arc in its ivar. Give the context its own
        // reference so a delayed IMK callback cannot outlive the gate.
        Arc::increment_strong_count(raw as *const InputContextGate);
        let input_context = &mut *input_context;
        input_context.set_ivar(INPUT_CONTEXT_GATE_IVAR, raw);
        (*this).set_ivar(TEXT_INPUT_CONTEXT_IVAR, input_context as *mut Object);
        input_context as *mut Object
    }
}

fn input_context_gate(object: &Object) -> Option<&InputContextGate> {
    let raw: *mut c_void = unsafe { *object.get_ivar(INPUT_CONTEXT_GATE_IVAR) };
    (!raw.is_null()).then(|| unsafe { &*(raw as *const InputContextGate) })
}

fn input_context_is_enabled(object: &Object) -> bool {
    input_context_gate(object).is_some_and(InputContextGate::is_enabled)
}

extern "C" fn activate_text_input_context(this: &Object, _: Sel) {
    if !input_context_is_enabled(this) {
        return;
    }

    unsafe {
        let _: () = msg_send![super(this, class!(NSTextInputContext)), activate];
    }
}

extern "C" fn deactivate_text_input_context(this: &Object, _: Sel) {
    // Never query TIS from here. TISGetInputSourceProperty can synchronously
    // ask AppKit for the current context and recurse back into this method.
    // Source and application notifications suspend the gate before teardown.
    if !input_context_is_enabled(this) {
        return;
    }

    unsafe {
        let _: () = msg_send![super(this, class!(NSTextInputContext)), deactivate];
    }
}

extern "C" fn handle_text_input_context_event(this: &Object, _: Sel, event: id) -> BOOL {
    if !input_context_is_enabled(this) {
        return NO;
    }

    unsafe { msg_send![super(this, class!(NSTextInputContext)), handleEvent: event] }
}

extern "C" fn invalidate_text_input_context_character_coordinates(this: &Object, _: Sel) {
    if !input_context_is_enabled(this) {
        return;
    }
    ...
    unsafe {
        let _: () = msg_send![
            super(this, class!(NSTextInputContext)),
...
unsafe fn drop_input_context_gate(object: &Object) {
    unsafe {
        let object = object as *const Object as *mut Object;
        let raw: *mut c_void = *(*object).get_ivar(INPUT_CONTEXT_GATE_IVAR);
        if !raw.is_null() {
            // AppKit can re-enter `deactivate` while its superclass tears down
            // the context. Clear the ivar before dropping the Arc so that
            // re-entry observes no gate rather than a dangling pointer.
            (*object).set_ivar(INPUT_CONTEXT_GATE_IVAR, ptr::null_mut::<c_void>());
            drop(Arc::from_raw(raw as *const InputContextGate));
        }
    }
}

unsafe fn drop_text_input_context(object: &Object) {
    unsafe {
        let object = object as *const Object as *mut Object;
        let input_context: id = *(*object).get_ivar(TEXT_INPUT_CONTEXT_IVAR);
        if !input_context.is_null() {
            (*object).set_ivar(TEXT_INPUT_CONTEXT_IVAR, nil);
            let _: () = msg_send![input_context, deactivate];
            let _: () = msg_send![input_context, release];
        }
    }
}
...
extern "C" fn dealloc_view(this: &Object, _: Sel) {
    unsafe {
        // Drop the context while both its client and its gate are still alive;
        // the context retains neither by itself.
        drop_text_input_context(this);
        drop_window_state(this);
        drop_input_context_gate(this);
        let _: () = msg_send![super(this, class!(NSView)), dealloc];
    }
}

extern "C" fn dealloc_text_input_context(this: &Object, _: Sel) {
    unsafe {
        drop_input_context_gate(this);
        let _: () = msg_send![super(this, class!(NSTextInputContext)), dealloc];
    }
}
